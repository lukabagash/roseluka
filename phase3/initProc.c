...
void test() {
    ...
    for(pid = 1; pid < UPROCMAX + 1; pid++) {
        ...
        for (i = 0; i < PGTBLSIZE; i++) {
            ...
            supportStruct[pid].sup_privatePgTbl[i].entryLO = ALLOFF | 0x00000400 | VALIDOFF | GLOBALOFF; /* Set entryLO with the frame number and write enabled, private to the specific ASID, and not valid */
        } 
