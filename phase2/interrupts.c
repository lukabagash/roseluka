...
void IOInt(){
	...
	/*can be made as a helper function(maybe)*/
	if (unblockedPcb == NULL){ /* if the supposedly unblocked pcb is NULL, we want to return control to the Current Process */
		if (currentProc != NULL){ /* if there is a Current Process to return control to */
			...
			loadProcessorState(currentProc); /* calling the function that returns control to the Current Process */
		}
...
void pltTimerInt(){
	...
	if (currentProc != NULL){ /* if there was a running process when the interrupt was generated */
		setTIMER(NEVER); /* PLT will generate an interrupt on interrupt line 1, loading the PLT with a very large value as we have acknowledged it (don't call PLT again) */
